...
enum class EventNumber(val rawValue: Int) {
    ...
    /** The strap's own battery-pack attach/detach signals — presence stated outright, ahead of any
     *  poll. Distinct from the pack's own event vocabulary, which reuses these numbers for other
     *  things on its own channel. */
...
enum class CommandNumber(val rawValue: Int) {
    ...
    // separate from the firmware-LOAD opcodes.
    ...
    // Connection + a confirmation. Never sent automatically.
    ...
    // a.b.c.d. Older 4.0 firmware replies "unsupported" (0a03) and is ignored.
    GET_HELLO(145),
    ...
    // Reversible; gated behind the deep-data opt-in.
    SET_CONFIG(120),
...
enum class RebootProbeVariant(
    ...
    /** Tag written to the strap log so each attempt is correlatable. */
    val logTag: String,
) {
