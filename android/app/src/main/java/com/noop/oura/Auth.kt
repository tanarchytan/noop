...
// Auth: the application-level challenge handshake. Kotlin twin of Auth.swift.
...
// The trailing 0x01 byte and the full-block 0x10 padding are load-bearing:
...
/**
 * Result of submitting the auth proof (status byte of the 0x2E response).
 */
...
object OuraAuth {
    ...
    /** The trailing marker byte appended to the nonce before encryption. */
    const val trailingMarker = 0x01

    /** PKCS full-block pad byte (a 16-byte block of value 0x10). */
    const val padByte = 0x10

    // MARK: - Outgoing commands

    /** Build the GetAuthNonce request: `2f 01 2b` (secure-session sub-op 0x01). */
     ...
     * `24 10 <16-byte key>`. Throws on a wrong-length key. This installs a
     ...
     *Throws on bad input lengths or a cipher failure. The padding is
    ...
    /** Build the Authenticate (submit-proof) command: `2f 11 2d <proof:16>`. */
    ...
    /** One-shot helper: nonce + key -> the ready-to-write Authenticate command.4-3.5. */
     ...
     *Returns null when the subop is not 0x2E or the status byte is absent.
     */
