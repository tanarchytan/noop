...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Re-arm the daily debug export so its schedule self-heals after a reboot or app update
        ...
        // Backup & Sync: self-heal the daily auto-backup schedule (no-op when off / no folder),
        // and run a DEFERRED on-launch catch-up backup. The catch-up is gated on the toggle
        ...
        // Load the Light/Dark/System preference before first composition so the theme is correct from
        // the very first frame (no flash).
...
object NoopPrefs {
    ...
    /** "Power saving": when on, NOOP stretches its periodic strap-sync cadence (15 → 45 min) while
    ...
    /** "Buzz WHOOP 4": arm the strap's firmware alarm at the phone smart alarm's earliest wake
    ...
    /** Last HR sample epoch-second exported to Health Connect (0 = nothing exported yet). The
    ...
    /** Per-weekday wake-time OVERRIDES: a map of Calendar.DAY_OF_WEEK
     ...
     *  entry can never schedule a bogus time. Empty = no overrides. */
    ...
    /** Battery alerts, low (≤15%) + charge-complete (100%) strap notifications (thanks @ujix).
    ...
    /** Scheduled report notifications, opt-in, default OFF, no AI. Two independent toggles:
    ...
    /** Caffeine late-intake nudge, opt-in, default OFF. When on, the Caffeine card
    ...
    /** Whether the one-shot implausible-timestamp heal has run. Set true once it completes so the
    ...
    /** RE-POLLUTION re-arm: set true by the BLE layer when a sync's ingest gate dropped implausible
     ...
     *  the next launch, e.g. after an APK update restarts the process. On-device only; never sent. */
    ...
    /** Wall-clock (unix seconds) of the last history offload that ran to HISTORY_COMPLETE. Persisted so
     *  the Live screen's "Last synced N ago" SURVIVES a BLE-client recreation / process restart and
     *  stops reverting to "Never". 0 = never synced on this install. */
