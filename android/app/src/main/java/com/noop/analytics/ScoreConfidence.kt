 ...
 *
 * Each daily score (Charge = recovery, Effort = strain, Rest = sleep_performance) carries a tier so a
 * sparse day reads truthfully instead of faking a number: CALIBRATING = the score can't honestly
 * compute yet (baseline unusable, no in-bed data, or no HR window) — usually paired with a null score;
 * BUILDING = usable but thin (e.g. < ~7 nights of baseline, or a 5/MG day whose HR is mostly
 * PPG-derived); SOLID = full inputs present. Surfaced as a small label/dot under each score; the
 * metric itself stays nil-honest where it can't compute.
 */
enum class ScoreConfidence(val raw: String) {
    ...
    companion object {
        ...
        /**
         * Rest (sleep performance) confidence. The tier reflects how complete THIS night's Rest
         * inputs are, not the sleep-need history length.
        ...
        // ── Stage low-confidence: restorative-share floor on a high-efficiency night ────────────────
         ...
         * flag LOW CONFIDENCE rather than fake stages or tank Rest.
         */
        ...
        /**
         * Rest confidence with the stage-quality check and the sparse-motion guard. Starts from
         * [forRest], then DOWNGRADES a [SOLID] tier to [BUILDING] (low-confidence) when EITHER:
         *  - the night was staged on SPARSE gravity ([gravitySparse]) — a WHOOP 4.0 offload night banks
         *    motion coarsely, too sparse to reliably stage sleep, so a confident 85–100 Rest is unearned
         *    however the engine filled the stages. This catches the case the restorative-share check
         *    alone misses: a sparse night whose staging manufactures HIGH efficiency AND HIGH
         *    restorative reads SOLID on that check alone, yet the underlying data can't support it; OR
         ...
         * [CALIBRATING]/[BUILDING] from the base call are returned unchanged. Confidence-only — it
         * never changes the Rest score or invents stages.
         */
        fun forRest(
            hasSession: Boolean,
            ...
            if (gravitySparse) return BUILDING   // sparse-motion staging can't earn a SOLID Rest
