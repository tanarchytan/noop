...
object SleepStagerV2 {
    ...
    private fun stageEpochs(feats: List<Epoch>): List<String> {
        ...
        for (f in feats) {
            ...
            // correction only ever holds a still, elevated-HR epoch. NB: noop-tan keeps the #348 DREAMT-tuned
            // deadzone term 0.5*dz(zhvv)+0.6*dz(zhrv); upstream #465 clamps over the post-#437 restored
            // 0.8*zhvv+0.4*zhrv, but we deliberately keep #348 (David's call), so the clamp wraps OUR term.
            val awakeCardiac0 = 0.5 * dz(zhvv) + 0.6 * dz(zhrv)
