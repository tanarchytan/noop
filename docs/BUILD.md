...
(`NOOP-android-*.apk`) is published in [Releases](https://github.com/tanarchytan/noop/releases). A
sample-data **mock** flavour ships alongside it (`NOOP-android-mock-*.apk`, `./gradlew assembleMockRc`)
for exploring every screen with no strap.
